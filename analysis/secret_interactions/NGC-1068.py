...
source = PointLikeSource(ra=np.deg2rad(alpha_NGC), dec=np.deg2rad(dec_NGC))
...
def model_logllh(index):
    ...
    si_analysis = create_si_analysis(cfg=cfg, datasets=datasets, source=source,
                                       E0 = 1e3,
...
result_df = pd.DataFrame(data={"M_phi": M, "g":G, "-2 logllh":-2 * (si_logllh - pl_log_lambda_max) })
result_df.to_csv("results/NGC_logllh.csv")
