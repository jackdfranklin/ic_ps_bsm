...
class PDSingleParamFluxPointLikeSourceI3DetSigYieldBuilder(
    ...
    def construct_detsigyield(
        ...
        def _create_hist(
            ...
            if(np.any(h < 0)):
                print("Negative counts calculated for flux:")
                print(str(fluxmodel.energy_profile))
                print("gamma = " + str(fluxmodel.energy_profile.gamma))

                print(sin_true_dec_binedges_lower[h < 0])
                print(sin_true_dec_binedges_upper[h < 0])

            h = np.where(h<=0, min_h*1e-10, h)
