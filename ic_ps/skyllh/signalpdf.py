...
class PDSignalEnergyPDFSet(
    ...
    def __init__(
        ...
        def create_energy_pdf(sm_pdf, fluxmodel, gridparams):
            ...
            flux_norm = my_fluxmodel.energy_profile.get_integral(
                            true_enu_binedges[0],
                            true_enu_binedges[-1]
                        )

            # If flux is 0, probability is 0 everywhere
            if flux_norm > 0.0 :
                flux_prob = (
                ...
                flux_prob = np.zeros(np.shape(true_enu_binedges_lower))
            ...
            if(np.all(p <= 0)):
                true_e_prob = np.zeros(np.shape(p))
            else:
