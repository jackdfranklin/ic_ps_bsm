    ...
    for( int s = 1; s <= N_z; ++s ) {
        ...
        if(s == N_z) {
            ...
            for ( mass_state i: {one, two, three} ) {
                apply_redshift(energy_nodes, current_flux.at(i), z, zs.at(s - 1));
            }
        }
        else {
            z = zs.at(s);
            delta_z = zs.at( s - 1 ) - z;
            for ( mass_state i: {one, two, three} ) {
                shift_bins(energy_nodes, current_flux.at(i));
            }
        }
        ...
        for( auto m = N_nodes - 1; m >= 0; --m ) {
                ...
                if ( m != N_nodes - 1 ) {
                    ...
                    for ( mass_state j: {one, two, three} ) {
                        ...
                        * ( Is.at(j).at(i).row(m).tail(N_nodes - ( m + 1 ) )
                            .transpose().array() 
                            * current_flux.at(j).tail(N_nodes - m - 1).array() 
            ...
            current_flux.at(one)(m) = sol(one);
            current_flux.at(two)(m) = sol(two);
            current_flux.at(three)(m) = sol(three);

        }


        for ( mass_state i: {one, two, three} ) {
            fix_values(deltaE_GeV, current_flux.at(i));
        }

    }

    Eigen::VectorXd muon_flux  = ( constants::PMNS_sq[mu][one]   * current_flux.at(one)  
                                + constants::PMNS_sq[mu][three] * current_flux.at(two) 
                                + constants::PMNS_sq[mu][two]   * current_flux.at(three) );
    return muon_flux.array() / deltaE_GeV.array();
